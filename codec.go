...
// inside the envelope payload. Implementations must be safe for concurrent use.
//
...
// slice passed to Decode is borrowed — implementations must not retain or
// mutate it past the Decode call.
...
// use. The output must be a valid non-empty Redis key.
...
// JSONCodec serializes V via encoding/json.
